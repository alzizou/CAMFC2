...
gama1 = 1e+2*diag([1e-0;1e-0]);
rho1 = 1e-0;

gama2 = 1e+1*diag([1e-0;1e-0]);
rho2 = 1e-0;
...
P_elems(6) = Simulink.BusElement;
P_elems(6).Name = 'p6';
P_elems(6).Dimensions = [n n];

P_bus = Simulink.Bus;
P_bus.Elements = P_elems;

P_InitVal = {zeros(n),zeros(n),zeros(n),zeros(n),zeros(n),zeros(n)};
...
T_elems(6) = Simulink.BusElement;
T_elems(6).Name = 't6';
T_elems(6).Dimensions = [(N+1) n];

T_bus = Simulink.Bus;
T_bus.Elements = T_elems;

T_InitVal = {zeros((N+1),n),zeros((N+1),n),zeros((N+1),n),zeros((N+1),n),zeros((N+1),n),zeros((N+1),n)};
...
epsilon_elems(6) = Simulink.BusElement;
epsilon_elems(6).Name = 'eps6';
epsilon_elems(6).Dimensions = [(N+1) n];

epsilon_bus = Simulink.Bus;
epsilon_bus.Elements = epsilon_elems;

epsilon_InitVal = {zeros((N+1),n),zeros((N+1),n),zeros((N+1),n),zeros((N+1),n),zeros((N+1),n),zeros((N+1),n)};
